...
import { sanitizeSave } from '../game/src/storage.mjs';
...
test('picture settings migrate old saves and reject nonfinite or out-of-range values', () => {
  assert.equal(sanitizeSave({ version: 1 }).settings.filmGrain, 0.35);
  assert.equal(sanitizeSave({ version: 1, settings: { filmGrain: -1, brightness: 8 } }).settings.brightness, 1.5);
  assert.equal(sanitizeSave({ version: 1, settings: { filmGrain: -1 } }).settings.filmGrain, 0);
  assert.equal(sanitizeSave({ version: 1, settings: { brightness: NaN } }).settings.brightness, 1);
});
