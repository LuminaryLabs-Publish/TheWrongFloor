...
export async function reviewAudio(run, reviewDir) {
  const result = await run(`(async () => {
    ...
    ambienceBus.connect(limiter); effectsBus.connect(limiter); limiter.connect(master); master.connect(context.destination);
    ...
    const steps = [
      [0, () => { event({type:'arrival',roundIndex:0}); update(normal); }],
