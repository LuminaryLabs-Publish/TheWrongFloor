export function createInput(canvas, callbacks = {}) {
  ...
  const keys = new Set();
  let mode = 'menu', pointerClose = false, drag = null, dx = 0, dy = 0;
  ...
  listen(window, 'keydown', event => {
    ...
    if (mode === 'intro') {
      if (!event.repeat && event.code === 'Enter') { event.preventDefault(); callbacks.onDescend?.(); }
      return;
    }
  ...
  listen(holdButton, 'pointerdown', event => {
    if (mode !== 'playing' || holdButton.disabled) return;
  ...
  function poll(nextSettings = {}) {
    ...
    if (edge(9) && mode === 'playing') callbacks.onPause?.();
    if (mode === 'menu') {
      ...
      if (edge(0)) callbacks.onDescend?.();
    } else if (edge(1)) callbacks.onRecenter?.();
    oldButtons = buttons;
    if (mode !== 'playing') { dx = 0; dy = 0; return { close: false, lookX: 0, lookY: 0 }; }
    ...
    dx = 0; dy = 0;
    return { close: keys.has(binding('close')) || pointerClose || Boolean(buttons[0]), lookX: Math.max(-3, Math.min(3, lookX)), lookY: Math.max(-3, Math.min(3, lookY)) };
  }
  return {
    poll, reset,
    setMode(value) { const next = ['menu','intro','playing'].includes(value) ? value : 'menu'; if (mode !== next) reset(); mode = next; },
    setMenu(value) { const next = value ? 'menu' : 'playing'; if (mode !== next) reset(); mode = next; },
    dispose() { reset(); abort.abort(); }
  };
}
