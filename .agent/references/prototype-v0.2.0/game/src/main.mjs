...
async function prepareIntro(seed){
  const token=++introToken;loading=true;introState.descendReady=false;introState.descendPressed=false;introState.introPhase='open';introState.introTime=0;introState.door.openness=1;introState.displayFloor=30;
...
function title(){
  ...
  document.body.classList.remove('intro-transition');input.reset();audio.pause();ui.show('title');scene.recenter();scene.render(introState,0,{},ui.getSettings());prepareIntro().catch(()=>{});
}
function descend(){
  if(game||preview||loading||!introState.descendReady||introState.introPhase!=='open')return false;
  audio.unlock();audio.setSettings(ui.getSettings());introState.descendReady=false;introState.descendPressed=true;introState.introPhase='closing';introState.introTime=0;document.body.classList.add('intro-transition');ui.setReady(false,'DESCENDING');previous=performance.now();return true;
}
...
function advanceIntro(dt){
  ...
  if(introState.introPhase==='closing'){
    ...
    if(introState.introTime>=INTRO_TIMING.travel){introState.introPhase='opening';introState.introTime=0;introState.displayFloor=29;}
...
async function start({seed,practice=false,manual:manualClock=false}={}){
  ...
  document.body.classList.remove('intro-transition');input.reset();audio.unlock();audio.setSettings(ui.getSettings());
  ...
  try{
    ...
    for(let i=0;i<schedule.length;i++){await scene.prepare(schedule[i],settings);if(token!==startToken||disposed)return;ui.setReady(false,`BUILDING STOP ${i+1} / ${schedule.length}`);}
...
function tick(now){
  ...
  scene.render(state,state.mode==='paused'?0:dt,game&&ui.getScreen()==='playing'?controls:{},settings);ui.update(state);audio.update(state);
  if(terminalAt&&!manual&&performance.now()-terminalAt>(settings.softScares?700:1600)){terminalAt=null;finish();}
}
function dispose(){if(disposed)return;disposed=true;startToken++;introToken++;cancelAnimationFrame(frame);input?.dispose();ui?.dispose();audio?.dispose();scene?.dispose();window.removeEventListener('pagehide',dispose);}
try{
  scene=createScene(document.getElementById('scene'));audio=createAudio();
  ui=createUI({descend,play:()=>start({}).catch(()=>{}),practice:()=>start({practice:true}).catch(()=>{}),resume,pause,title,retry:()=>start({}).catch(()=>{}),recenter:()=>scene.recenter(),exit:()=>{if(desktop)window.close();else if(window.parent!==window){title();ui.setReady(true,'USE THE LIBRARY CLOSE BUTTON TO EXIT');}else location.assign('../../');},screenChanged:setInputMode,settingsChanged:settings=>{save.settings=settings;writeSave(save);audio.setSettings(settings);}});
  input=createInput(document.getElementById('scene'),{onPause:pause,onBlur:()=>{if(game?.snapshot().mode==='running')pause();},onRecenter:()=>scene.recenter(),onDescend:descend,onIntroPointer:(x,y)=>scene.hitTestDescend(x,y)&&descend(),onConfirm:()=>ui.confirm(),onMenuMove:delta=>ui.menuMove(delta)});
  ui.updateSettings(save.settings);audio.setSettings(save.settings);ui.show('title');ui.setReady(false);scene.render(introState,0,{},save.settings);
  ...
  if(review)window.__wrongFloor={
    start,descend,snapshot:()=>preview??game?.snapshot()??introState,inspect:()=>scene.inspect(),
