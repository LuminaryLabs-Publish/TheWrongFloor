import { AUDIO_ASSETS, SAMPLE_LEVELS, clamp01, hallwayMix, monsterVisibleForAudio } from './audio-config.mjs';
import { conditionChannels, findTransientStart } from './audio-processing.mjs';
...
export function createSampleAudio({ context: ctx, ambienceBus, effectsBus, getSettings }) {
  ...
  function update(snapshot = {}) {
    if (disposed) return;
    const running = snapshot.mode === 'running', mix = hallwayMix(snapshot);
    const soft = getSettings().softScares ? 0.58 : 1;
    if (!running) { if (music) smooth(music.gain.gain, 0, 0.15); return; }
    if (!music) music = createSource('musicBox', ambienceBus, { loop: true, gain: 0, pan: -0.18, cutoff: 3400 });
