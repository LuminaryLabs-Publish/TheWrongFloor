...
export function createProceduralAudio({ context: ctx, ambienceBus, effectsBus, getSettings }) {
  ...
  let disposed = false, seed = 617, stepKey='';
  ...
  function update(snapshot = {}) {
    if (disposed) return;
    const running = snapshot.audible ?? (snapshot.mode === 'running' || snapshot.mode === 'lobby'), travel = snapshot.phase === 'travel';
    const door = snapshot.doorMoving ?? ['opening','closing','entering'].includes(snapshot.phase);
    const pry=snapshot.mode==='lobby' && snapshot.phase==='pry' && !getSettings().softScares;
    const hush = snapshot.clueVisible && !snapshot.resolved ? 0.6 : 1;
    smooth(motorGain.gain, running ? (travel ? 0.17 : door ? 0.11 : pry ? 0.065 : 0.035) * hush : 0);
    smooth(filter.frequency, travel ? 210 : door ? 760 : 340);
    smooth(humGain.gain, running ? (travel ? 0.027 : 0.012) * hush : 0);
    const room=snapshot.round?.profile??'';let roomTone=0;for(const c of room)roomTone=(roomTone+c.charCodeAt(0))%11;
    smooth(hum.frequency, travel ? 48 : 48+roomTone);
    if(snapshot.mode==='running'&&snapshot.clueVisible&&!snapshot.resolved&&['warden','weaver','mourner'].includes(snapshot.round?.entity)){
      const age=Math.max(0,(snapshot.roundTime??0)-(snapshot.round.clueAt??0)),step=Math.floor(age*5.2/Math.PI),key=`${snapshot.roundIndex}:${step}`;
      if(stepKey!==key){stepKey=key;knock(.035+Math.min(1,snapshot.threatProgress??0)*.04);}
    }
  }
  function event(entry = {}) {
    const soft = getSettings().softScares ? 0.55 : 1;
    if (entry.type === 'lobby-closing') knock(0.035);
    if (entry.type === 'lobby-pry' && !getSettings().softScares) {tone(147,0.009,1.1,0,122);tone(213,0.004,1.4,0.2,183);}
    if (entry.type === 'lobby-retreat' && !getSettings().softScares) tone(190,0.004,0.5,0,110);
    if (entry.type === 'lobby-entering') tone(440,0.012,0.45);
