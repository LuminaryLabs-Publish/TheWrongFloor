...
export function findTransientStart(channels, sampleRate) {
  ...
  const windows=[];let loudest=0;
  for(let i=0;i<(channels[0]?.length??0);i+=size){let energy=0;
    for(const channel of channels)for(let j=i;j<Math.min(i+size,channel.length);j++)energy+=channel[j]**2;
    windows.push({i,energy});loudest=Math.max(loudest,energy);
  }
  if(loudest<1e-8)return 0;
  const first=windows.find(w=>w.energy>=loudest*.08);
  return Math.max(0,(first?.i??0)-Math.floor(sampleRate*.09));
}
