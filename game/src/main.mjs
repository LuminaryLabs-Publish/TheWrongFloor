...
function tick(now){if(disposed)return;frame=requestAnimationFrame(tick);const dt=Math.min(.1,Math.max(0,(now-(previous||now))/1000));previous=now;
  ...
  if(lobby&&!game&&!preview){const s=entry.update(dt);lobbyAudio.update(s);lobby.update(s,settings);const canvas=document.getElementById('scene'),size=`${canvas.clientWidth}:${canvas.clientHeight}`;if(size!==lobbySize){lobbySize=size;lobby.resize(canvas.clientWidth,canvas.clientHeight);}scene.drawExternal(lobby.scene,lobby.camera);audio.update({mode:'lobby',phase:s.stage,door:{openness:s.openness},clueVisible:false,threatProgress:0});}
...
function dispose(){window.removeEventListener('focus',onFocus);if(disposed)return;disposed=true;startToken++;cancelAnimationFrame(frame);entry?.dispose();lobby?.dispose();input?.dispose();ui?.dispose();audio?.dispose();scene?.dispose();window.removeEventListener('pagehide',dispose);}
try{
  ...
  input=createInput(document.getElementById('scene'),{onPause:pause,onBlur:()=>{audio.pause();if(game?.snapshot().mode==='running')pause();},onRecenter:()=>scene.recenter(),onConfirm:()=>ui.confirm(),onMenuMove:delta=>ui.menuMove(delta)});
  ...
  if(review)window.__wrongFloor={
    start,snapshot:()=>game?.snapshot()??titleState,lobby:()=>entry.snapshot(),inspect:()=>({...scene.inspect(),audio:audio.inspect()}),
