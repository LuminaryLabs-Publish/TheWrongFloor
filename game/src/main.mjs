...
import {createLobbyAudioPolicy} from '../floor-30/audio.mjs';
...
let lobby=null,entry=null,lobbySize='',lobbyAudio=null;
const openingListeners=new AbortController();
function unlockOpening(){if(!game){audio?.resume();audio?.unlock();}}
window.addEventListener('pointerdown',unlockOpening,{capture:true,signal:openingListeners.signal});
window.addEventListener('keydown',unlockOpening,{capture:true,signal:openingListeners.signal});
document.addEventListener('visibilitychange',()=>{if(document.hidden)audio?.pause();else if(!game)audio?.resume();},{signal:openingListeners.signal});
const cinematic=createCinematic({settings:save.settings,skip:review&&params.get('skipIntro')==='1',onComplete:()=>{previous=performance.now();document.getElementById('play-button')?.focus({preventScroll:true});}});
...
function pause(){if(cinematic.active||loading)return;if(ui.getScreen()==='settings'){ui.show(game?.snapshot().mode==='paused'?'pause':'title');return;}if(game?.snapshot().mode==='running'){game.pause();input.reset();audio.pause();ui.show('pause');}else if(game?.snapshot().mode==='paused')resume();}
function resume(){if(!game)return;game.resume();input.reset();audio.resume();previous=performance.now();ui.show('playing');}
function title(){startToken++;loading=false;game=null;preview=null;terminalAt=null;manual=false;input.reset();entry?.reset();lobbyAudio?.reset();audio.reset();audio.resume();ui.show('title');ui.setReady(true);}
async function start({seed,practice=false,manual:manualClock=false}={}){
  ...
  input.reset();audio.reset();audio.resume();audio.setSettings(ui.getSettings());await audio.unlock();if(token!==startToken||disposed)return;
...
function tick(now){if(disposed)return;frame=requestAnimationFrame(tick);const dt=Math.min(.1,Math.max(0,(now-(previous||now))/1000));previous=now;
  document.getElementById('intro-sound').hidden=!cinematic.active||audio.inspect().state==='running';
  audio.updateOpening({...cinematic.inspect(),menu:!game&&!loading&&!preview&&!document.hidden});
  if(cinematic.active&&!cinematic.blending)return;
  ...
  if(lobby&&!game&&!preview){const s=entry.update(cinematic.active?0:dt);lobbyAudio.update(s);lobby.update(s,settings,dt);const canvas=document.getElementById('scene'),size=`${canvas.clientWidth}:${canvas.clientHeight}`;if(size!==lobbySize){lobbySize=size;lobby.resize(canvas.clientWidth,canvas.clientHeight);}scene.drawExternal(lobby.scene,lobby.camera);audio.update({mode:'lobby',phase:s.stage,door:{openness:s.openness},clueVisible:false,threatProgress:0});}
...
function dispose(){window.removeEventListener('focus',onFocus);if(disposed)return;disposed=true;startToken++;cancelAnimationFrame(frame);openingListeners.abort();cinematic.dispose();entry?.dispose();lobby?.dispose();input?.dispose();ui?.dispose();audio?.dispose();scene?.dispose();window.removeEventListener('pagehide',dispose);}
try{
  scene=createScene(document.getElementById('scene'));audio=createAudio();lobbyAudio=createLobbyAudioPolicy(e=>audio.event(e));
  ...
  input=createInput(document.getElementById('scene'),{onPause:pause,onBlur:()=>{audio.pause();if(game?.snapshot().mode==='running')pause();},onRecenter:()=>scene.recenter(),onConfirm:()=>{if(!cinematic.active)ui.confirm();},onMenuMove:delta=>{if(!cinematic.active)ui.menuMove(delta);}});
  ...
  lobby=await createLobbyScene(THREE,loadLobbyAsset,{authored:await scene.prepareLobby(),loadIntro:loadLobbyAsset});
  entry=createLobbyEntry({button:document.getElementById('play-button'),isActive:()=>!cinematic.active&&!game&&!loading&&ui.getScreen()==='title',onGesture:()=>{audio.resume();audio.unlock();},onAccept:()=>start({seed:document.getElementById('seed-input').value.trim()}).catch(()=>{})});
  lobby.update(entry.snapshot(),save.settings);lobby.resize(innerWidth,innerHeight);scene.drawExternal(lobby.scene,lobby.camera);ui.setReady(true);cinematic.markReady();
  ...
  if(review)window.__wrongFloor={
    start,snapshot:()=>game?.snapshot()??titleState,lobby:()=>entry.snapshot(),inspect:()=>({...scene.inspect(),lobby:lobby?.inspect(),audio:audio.inspect(),cinematic:cinematic.inspect()}),
